...
const Step2Interview = ({ interviewData, onFinish }) => {
    ...
    const startMic = () => {
        ...
        if (!recognition || isAIPlaying || isMicRunningRef.current) return;

        try {
            recognition.start();
        } catch (err) {
            console.warn("Mic prevented:", err.message);
        }
    };
    ...
    const submitAnswer = async () => {
        ...
        try {
            const result = await axios.post(serverURL + "/api/interview/submit-answers", {
                interviewId,
